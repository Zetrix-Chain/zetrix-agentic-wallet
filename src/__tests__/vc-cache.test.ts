...
import { createFsVcCache, isVcValid, extractValidUntil, extractAttributeKeys } from '../clients/vc-cache'
...
// A cached VC's claims live nested under credentialSubject.<wrapper> (e.g. `aiBirthcert`), not
// at the top of credentialSubject — comparing a held VC against a template's declared schema
// needs those inner keys. See the staleAttributes check in subscribe.ts.
describe('extractAttributeKeys', () => {
  const vc = {
    credentialSubject: {
      id: 'did:zid:holder',
      aiBirthcert: { agentUsername: 'jak', ownerName: 'Izzatur', dob: '2000-01-01', id: 'jak' },
    },
  }

  it('returns the claim keys nested under the credentialSubject wrapper', () => {
    expect(extractAttributeKeys(vc).sort()).toEqual(['agentUsername', 'dob', 'id', 'ownerName'])
  })

  it('ignores credentialSubject.id, which identifies the holder rather than being a claim', () => {
    expect(extractAttributeKeys(vc)).not.toContain('did:zid:holder')
  })

  it('merges keys across multiple wrappers', () => {
    const multi = { credentialSubject: { id: 'x', a: { one: 1 }, b: { two: 2 } } }
    expect(extractAttributeKeys(multi).sort()).toEqual(['one', 'two'])
  })

  it('returns an empty list for a VC with no usable credentialSubject', () => {
    expect(extractAttributeKeys({})).toEqual([])
    expect(extractAttributeKeys(null)).toEqual([])
    expect(extractAttributeKeys({ credentialSubject: { id: 'only-holder' } })).toEqual([])
  })
})
