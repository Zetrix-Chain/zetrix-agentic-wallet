...
/**
 * The claim keys a VC actually carries. They live nested under `credentialSubject.<wrapper>`
 * (e.g. `credentialSubject.aiBirthcert.agentUsername`), not at the top of `credentialSubject`,
 * so a plain `Object.keys(credentialSubject)` returns the wrapper name rather than the claims.
 * `credentialSubject.id` is skipped — it identifies the holder, it is not a claim.
 *
 * Used to compare a held VC against a template's currently-declared schema: `isVcValid` only
 * answers "has it expired?", which leaves a VC whose fields no longer match the template
 * looking perfectly good.
 */
export function extractAttributeKeys(vc: unknown): string[] {
  ...
  for (const [key, value] of Object.entries(subject as Record<string, unknown>)) {
    ...
    if (typeof value === 'object' && value !== null) {
      for (const inner of Object.keys(value as Record<string, unknown>)) keys.add(inner)
    }
  }
  return [...keys]
}
