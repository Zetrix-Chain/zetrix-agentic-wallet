...
describe('manifest', () => {
  ...
  // OpenClaw materialises configSchema defaults into plugin config with no operator action
  // (SPIKE-0.4-FINDINGS.md §2.1, observed live). So any default declared here SHADOWS the wallet's
  // own — which is network-aware and the only place that can be. Declaring none is what lets the
  // wallet decide, and is why this is asserted rather than merely left out.
  it('declares NO payment-cap default, so the wallet decides it per network', () => {
    expect(manifest.configSchema.properties.maxPaymentAmount.default).toBeUndefined()
  })

  // The cap's actual behaviour is asserted behaviourally in the wallet's own config.test.ts, by
  // calling loadConfig — this project cannot import the wallet's ESM modules, and regexing
  // config.ts (as this test used to) would pass on a rewrite that kept the shape and changed the
  // behaviour, which is the one failure a safety guard must not have.
  //
  // What belongs HERE is the manifest's promise to the subscriber matching what the wallet does.
  // The hint previously said mainnet refuses every payment; the credential allowance now reaches
  // mainnet, so that sentence would be a lie a subscriber acts on when deciding whether to set a
  // limit of their own.
  it('does not promise the subscriber that mainnet refuses every payment', () => {
    const hint = manifest.configUiHints.maxPaymentAmount.help as string
    expect(hint).not.toMatch(/refuses every payment on mainnet/i)
    // It must still say a limit applies per payment rather than as a running total, since that is
    // the property people misread as a budget.
    expect(hint).toMatch(/per payment/i)
  })
...
describe('SKILL.md', () => {
  it('names only tools the wallet actually exposes', () => {
    ...
    const unknown = toolish.filter(
      // Result/error codes and commands the skill legitimately names, which are not tools.
      (n) =>
        !walletToolNames.includes(n) &&
        !['not_activated', 'resource_payment', 'export_credentials', 'query_failed', 'unknown_token'].includes(n),
    )
  ...
  // computed a figure from the conversation's own spend history and presented it with the same
  // confidence as a verified read. A wrong balance stated confidently is worse than no balance.
  it('forbids reconstructing a balance from conversation history when a lookup fails', () => {
    expect(skill).toMatch(/query_failed|unknown_token/)
    expect(skill).toMatch(/never (invent|infer|compute|reconstruct|estimate)[^.]*balance|balance[^.]*never (invent|infer|compute|reconstruct|estimate)/i)
  })

  // The captured session asked for optional metadata and a required agent name across two turns
  // before any mention of price — cost surfaced only when the issuance attempt failed. Disclosure
  // "before spending" was not enough; it has to be before COLLECTING.
  // The runtime contract already banned editing OpenClaw config — and the captured session did it
  // anyway, then restarted the gateway and dropped the MCP connection for ~10 minutes. A single rule
  // at the top of the file was not enough; the prohibition has to be restated at the one moment it
  // is tempting, which is a payment refused for exceeding the cap.
  it('repeats the no-config-edit rule where a cap refusal is handled, not only in the runtime contract', () => {
    const capSection = skill.slice(skill.indexOf('payment cap'))
    expect(capSection).toMatch(/do not (run|edit|restart)|never (run|edit|restart)/i)
    expect(capSection).toMatch(/restart/i)
  })

  it('requires preflight before collecting any application detail', () => {
    expect(skill).toContain('credential_preflight')
    expect(skill).toMatch(/before (you )?(collect|ask)/i)
  })

  it('tells the agent to relay what preflight could not check, so a clean result is not a guarantee', () => {
    expect(skill).toMatch(/notChecked/)
  })

  it('tells the agent to quote the human-readable amount, not the raw base units', () => {
    // Must reference the `display` FIELD, not the word "display" — which already appears in the
    // unrelated "never display a private key" rule.
    expect(skill).toMatch(/`display`/)
  })
