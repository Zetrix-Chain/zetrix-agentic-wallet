...
describe('createHolderAccount', () => {
  it('creates a new HSM account, saves it, and returns address/DID when no account exists yet', async () => {
    const create = vi.fn().mockResolvedValue({ zetrixAddress: 'ZTX3New', publicKeyHex: `b001${rawPart}4a10ec51`, activated: true, activationTxHash: '0xabc' })
  ...
  it('polls checkActivationStatus and reports activated:true when it becomes activated', async () => {
    const create = vi.fn().mockResolvedValue({ zetrixAddress: 'ZTX3New', publicKeyHex: `b001${rawPart}4a10ec51`, activated: false, activationTxHash: null })
    const getExistingAccount = vi.fn().mockResolvedValue(null)
    const saveAccount = vi.fn().mockResolvedValue(undefined)
    ...
    expect(out.activated).toBe(true)
    expect(out.message).not.toMatch(/activation/i)
  })

  it('reports activated:false and notes it in message when polling never activates', async () => {
    const create = vi.fn().mockResolvedValue({ zetrixAddress: 'ZTX3New', publicKeyHex: `b001${rawPart}4a10ec51`, activated: false, activationTxHash: null })
    const getExistingAccount = vi.fn().mockResolvedValue(null)
    const saveAccount = vi.fn().mockResolvedValue(undefined)
    ...
    expect(out.message).toMatch(/activation/i)
  })
})
