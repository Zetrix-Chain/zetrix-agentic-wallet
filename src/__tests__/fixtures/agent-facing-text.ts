...
export const EXPECTED_AGENT_TEXT: Record<string, Record<string, readonly string[]>> = {
  ...
  "request_ai_birthcert_verification": {
    "description": [
      ...
      "Returns { sessionId, verificationUrl, expiresAt } — show verificationUrl to the human owner and ask them to open it and complete MyDigital ID verification (typically finishes in seconds).",
    ...
    "inputSchema.properties.discardStuckReceiptAndPayFresh.description": [
      ...
      "A receipt only counts as stuck once it is older than SETTLEMENT_STUCK_AFTER_MS (24h by default, e.g. SETTLEMENT_STUCK_AFTER_MS=3600000 for one hour); before that the wallet will say the settlement may still be in flight, and starting over is the user decision, not yours.",
    ],
  },
  ...
  "check_ai_birthcert_verification": {
    "description": [
      ...
      "While the session is still open the result carries `verificationUrl` (the same link issued at creation) and `expiresAt` — give the user both, so they know how long it is good for.",
