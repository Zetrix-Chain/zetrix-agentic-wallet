...
describe('buildToolList', () => {
  it('exposes exactly the 10 agent tools with the correct required inputs', () => {
    const tools = buildToolList()
    expect(tools.map((t) => t.name).sort()).toEqual([
      'check_ai_birthcert_verification', 'create_holder_account', 'credential_preflight', 'get_template_schema',
      'pay_and_fetch', 'prove_identity', 'query_contract', 'request_ai_birthcert_verification', 'subscribe_and_issue',
      'wallet_status',
    ])
