...
export interface CreateHolderAccountResult {
  ...
  holderDid?: string
  publicKeyHex?: string
  /** Final on-chain activation state after a bounded poll — present only when created is true. */
  activated?: boolean
  message: string
}
...
export async function createHolderAccount(deps: CreateHolderAccountDeps, input: CreateHolderAccountInput): Promise<CreateHolderAccountResult> {
  ...
  const message =
    ...
    (finalActivated
      ? ''
      : ` Note: on-chain activation has not completed yet — balance/on-chain calls for this address may fail ` +
        `until it does; check again later.`)
