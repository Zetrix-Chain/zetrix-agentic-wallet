 ...
 * reading `accepts[0].asset`.
 *
 * `opts.dryRun` does NOT run the flow above at all. It prices via `/v1/vc/pay/quote`
 * and returns before step 2, so it never signs and never reaches `applyChallenge`. That matters
 * because step 3 is not always just a quote: against a template MBI issues for free,
 * `applyChallenge` IS the real synchronous issuance, so stopping "after phase 1" would mean
 * stopping after the credential already exists. `/quote` cannot issue, which is why pricing goes
 * there instead.
 *
...
export interface SubscribeOpts {
  ...
  /**
   * Price the credential via `/v1/vc/pay/quote` and return, without signing, paying or issuing.
   * Does not reach `applyChallenge` — against a free template that call issues synchronously
...
export interface Quote {
  ...
  payTo?: string
  /** Which side pays network gas — always stated, so a missing value is never mistaken for self-pay. */
  gasModel?: 'sponsored' | 'self'
  /**
   * Whether MBI will actually charge `maxAmountRequired`. `false` means issuance is
   * currently free and the amount above will not be taken.
   *
   * ABSENT means unknown — an older MBI deployment without this field — and must be reported as an
   * unconfirmed amount, never as free. Three states, not a boolean with a default.
   */
...
export interface SubscribeResult {
  ...
  txHash?: string
  reason?: string
  /** The `/v1/vc/pay/quote` price — present when `dryRun` is set (asset symbol resolved from chain). */
  quote?: Quote
...
export async function subscribeAndIssue(deps: SubscribeDeps, opts: SubscribeOpts): Promise<SubscribeResult> {
  ...
  // A dry run prices via `/v1/vc/pay/quote` and stops here — it must never reach
  // `applyChallenge` below. For a FREE template `applyChallenge` is the real synchronous issuance,
  // so pricing through it mints a permanent VC, spends one of the holder's rate-limited free-mode
  // issuances, and evicts whatever this template's cache slot held — all as the side effect of
  // asking what something costs. `/quote` cannot issue, which is the whole reason it is used here.
  // It also needs no holder signature, so a dry run never touches the signing key.
  if (opts.dryRun) {
    // Falling back to `applyChallenge` without a quote client would reintroduce exactly the bug
    // this branch exists to prevent, so an unavailable quote client fails the dry run instead.
    if (!deps.mbi.quote) {
      return {
        issued: false,
        reason:
    ...
    let quoted
    try {
      quoted = await deps.mbi.quote(opts.templateId, attributes as Record<string, unknown>)
    } catch (err) {
    ...
    // Three distinct states, deliberately not collapsed: charged, free, and unknown. Reporting an
    // An older MBI without this field as "free" would be a guess; reporting it as a certain charge is
    // an overstatement this design avoids. So say the amount is unconfirmed.
    const reason =
      quoted.paymentRequired === false
        ? 'dry run — issuance is free right now, no payment required'
        : quoted.paymentRequired === true
          ? 'dry run — quoted only, no payment made'
    ...
    return {
      ...
      quote: {
        ...
        ...(quoted.paymentRequired === undefined ? {} : { paymentRequired: quoted.paymentRequired }),
      },
      ...(schema ? { schema } : {}),
    }
  }
  ...
  // This has already happened on chain by the time we observe it. Unreachable from a dry run
